...
class SignUpViewController: UIViewController, KeyboardNotifierDelegate, UITextFieldDelegate {
    ...
    @IBAction func btnLogInTapped(sender: UIButton?){
        ...
        DataValidator.validateAuthData(self.txtEmail.text!, password: self.txtPass.text!){ (isValid) in
            ...
            AuthNetWorker.registerWithData(self.txtEmail.text, pass: self.txtPass.text, completionHandler: { (isDone, errorText) in
